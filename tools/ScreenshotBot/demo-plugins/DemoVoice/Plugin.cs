...
namespace TuneLab.DocsPlugins.DemoVoice;

// 文档示例声源（docs/user-manual.zh-CN.md 的插图素材）：从 tests/plugins/V1.Voice 的参照实现裁剪而来——
// 去掉测试专用的声库/参数/失败钩子，换成面向读者的中文命名。合成 = 按音符音高填一段正弦，
// 并产出按音符归属的音素，够手册展示界面用。
public sealed class DemoVoiceEngine : IVoiceSynthesisEngine
{
    ...
    [
        VoiceSourceLayoutItem.Voice("clear"),
        VoiceSourceLayoutItem.Voice("warm"),
    ];

    public void Init()
    {
        mVoiceInfos.Add("clear", new VoiceSourceInfo { Name = "示例声源 · 澄", Description = "文档示例音色：明亮、清澈" });
        mVoiceInfos.Add("warm", new VoiceSourceInfo { Name = "示例声源 · 暖", Description = "文档示例音色：温暖、厚实" });
        // 音符属性：一个带端点描述的滑条 + 一个可拖写的数值框。
        mNoteProperties.Add(("tension", "张力"), SliderConfig.Linear(0, -1, 1).WithMinLabel("松").WithMaxLabel("紧"));
        mNoteProperties.Add(("velocity", "力度"), DraggableNumberBoxConfig.Integer(64).WithRange(0, 127));
        // 音素属性：核（元音）多一个重音，辅音只有偏移——演示按 slot 差异化的音素属性面板。
        mCorePhonemeProperties.Add(("accent", "重音"), SliderConfig.Linear(0, 0, 1).WithMaxLabel("强"));
        var offset = DraggableNumberBoxConfig.Create(0).WithSensitivity(0.5).WithFormat(NumberFormat.Decimals(1));
        mCorePhonemeProperties.Add(("offset", "偏移 (ms)"), offset);
        mConsonantPhonemeProperties.Add(("offset", "偏移 (ms)"), offset);
        // 片段属性开关：取消勾选时「气声」参数轨随之隐藏（已画的曲线保留）。
        mPartProperties.Add(("growl_enabled", "启用气声"), CheckBoxConfig.Create(true));
        mGrowlConfigs.Add(("Growl", "气声"), AutomationConfig.Create(0, 100).WithColor("#E5A573").WithDefault(0)
            .WithMinLabel("平直").WithMaxLabel("沙哑"));
    }

    public void Destroy() { }

    public IVoiceSynthesisSession CreateSession(IVoiceSynthesisContext context) => new DemoVoiceSession(context);
    ...
    public IReadOnlyOrderedMap<PropertyKey, AutomationConfig> GetAutomationConfigs(IVoiceSynthesisPartPropertyContext context)
    {
        ...
        map.Add(("Bend", "弯音"), mBendConfig);
        // 与回显轨配对的实参轨（同 Id "energy"、显示名不同）+ 不配对的偏差轨：验证「模型建议 + 用户覆盖」这组
        // 分工——普通用户调偏差轨（Energy），重度用户切到实参轨（Energy (Actual)）在模型输出这条底上直接覆盖。
        map.Add(("energy", "能量（实参）"), mEnergyActualConfig);
        map.Add(("energy_offset", "能量偏移"), mEnergyOffsetConfig);
        return map;
    }
    ...
    {
        { ("energy", "能量"), AutomationConfig.Create(0, 100).WithColor("#E573B0") },
    };
}

public sealed class DemoVoiceSession : IVoiceSynthesisSession
{
    public DemoVoiceSession(IVoiceSynthesisContext context)
    {
        mContext = context;
